...
export function ReceivePoForm({ poId, items }: Props) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
      <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Receive stock</h3>
        ...
        {items.map((it) => {
          ...
          return (
            <div key={it.id} className="flex flex-wrap items-center gap-3">
              <span className="min-w-[140px] text-sm text-zinc-800 dark:text-zinc-200">
                {it.product_name}
              </span>
              ...
              <input
                ...
                placeholder="0"
                className="w-28 rounded-lg border border-zinc-200 px-2 py-1 text-sm tabular-nums dark:border-zinc-700 dark:bg-zinc-900"
              />
            </div>
          );
        })}
