import { RestaurantWaiterCreateForm } from "@/components/restaurant/waiter-create-form";
...
export default async function RestaurantWaitersPage() {
  ...
  const waiters = (rows ?? []) as { id: string; name: string; phone: string | null; is_active: boolean }[];

  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">Waiter management</h1>
      <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Assign and maintain your serving staff list.</p>

      <div className="mt-6 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <RestaurantWaiterCreateForm />
      </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {waiters.length === 0 ? (
              ...
              waiters.map((w) => (
                <tr key={w.id}>
                  <td className="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-50">{w.name}</td>
                  <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300">{w.phone ?? "—"}</td>
                  <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300">{w.is_active ? "Yes" : "No"}</td>
                </tr>
              ))
            )}
          </tbody>
