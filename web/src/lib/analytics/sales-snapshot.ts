...
/** How many individual product slices the pie may show; the rest are merged into "Other" so the chart stays readable and still sums to 100% of line revenue. */
export const PRODUCT_MIX_MAX_NAMED_SLICES = 8;

export type ProductMixRow = {
  name: string;
  revenue: number;
  quantity: number;
  /** When true, the UI should show the translated "all other products" label instead of `name`. */
  isOther?: boolean;
};
...
export type SalesSnapshot = {
  ...
  orderCount: number;
  averageOrder: number;
  /** One row per calendar day in range (revenue 0 if none). */
  daily: { dateKey: string; label: string; revenue: number }[];
  /** Mix for charts: up to {@link PRODUCT_MIX_MAX_NAMED_SLICES} products plus optional aggregate "other". */
  topProducts: ProductMixRow[];
  /** Sum of all invoice line totals in the period; denominator for % share in product mix. */
...
/**
 * Invoices in range with finalized or open payment states; aggregates daily revenue and line-level top products.
 */
