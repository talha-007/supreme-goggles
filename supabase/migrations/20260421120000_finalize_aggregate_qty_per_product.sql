-- Deduct stock once per catalog product using sum(quantity), so duplicate invoice_items rows
-- (same product_id) cannot double the movement. Matches merge on the web save path.
