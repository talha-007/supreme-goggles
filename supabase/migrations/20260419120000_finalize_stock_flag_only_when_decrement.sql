-- Only set stock_deducted_at when at least one catalog line decremented stock.
-- Previously the flag was set even for free-text-only invoices (no product_id), so reverse tried to
-- "restore" nothing and inventory looked broken.
