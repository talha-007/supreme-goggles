-- Restore on void when finalized invoice has catalog lines (product_id + qty), even if
-- stock_deducted_at is null and no sale stock_movements exist (legacy / missing audit rows).
